...
const VenuePartnerCard: React.FC<VenueProps> = ({
  ...
  const confirmDeletion = async () => {
    ...
    try {
      ...
      await deleteVenue.mutateAsync({ id: data.id });
    } catch (error) {
  ...
  return (
    <>
      <div
        className="w-full px-4 pb-4 pt-16 flex flex-col items-center text-center gap-2 rounded-[6px] hover:border-[#85009D] border border-[#DBBB89] hover:bg-[#85009D] bg-[#FFFBF5] text-[#85009D] hover:text-white transition-all duration-200 ease-in-out group relative"
      >
