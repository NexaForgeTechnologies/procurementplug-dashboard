...
const PdfUploader: React.FC<PdfUploaderProps> = ({ onUpload, defaultFile }) => {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // ⬇️ Load default file when component mounts or changes
  useEffect(() => {
    if (defaultFile) {
      setSelectedFile(defaultFile);
      onUpload(defaultFile); // Optional: only call if not already synced
    }
  }, [defaultFile]);
