...
type EditTalentProps = {
  active: boolean;
  talent: TalentHiringDM;
  onClose: () => void;
  refetchTalents: () => void;
};

const EditTalentCard: React.FC<EditTalentProps> = ({
  active,
  talent,
  onClose,
  refetchTalents,
}) => {
  const [formValues, setFormValues] = useState<TalentHiringDM>(talent);
  ...
  const updateTalentMutation = useMutation({
    mutationFn: async (data: TalentHiringDM) => {
      const response = await axios.put(`/api/talent-hiring-intelligence/professionals/`, data);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedFile) {
        // Optional: If you want to delete old image, implement here before uploading new one
    ...
    <div className="fixed inset-0 bg-black/70 z-50 px-4">
      <div className="max-w-md max-h-[90vh] overflow-y-auto py-4 px-3 bg-[#F7F9FB] relative top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 rounded-md">
        ...
        <div className="flex justify-between items-center">
          <h2 className="font-medium text-2xl text-[#565656]">Edit Talent</h2>
